...
def create_task(task: todo_schema.TodoCreate, user: user_schema.User):
    ...
    return todo_schema.Todo(
        id=db_task.id, title=db_task.title, is_done=db_task.is_done, created_at=db_task.created_at
    )
...
def get_tasks(user: user_schema.User, is_done: bool = None):
    ...
    if (is_done is None):
        ...
        tasks_by_user = TodoModel.filter((TodoModel.user_id == user.id)
                                         & (TodoModel.is_done == is_done)).order_by(
...
def get_task(task_id: int, user: user_schema.User):
    ...
    return todo_schema.Todo(id=task.id, title=task.title, is_done=task.is_done, created_at=task.created_at)
